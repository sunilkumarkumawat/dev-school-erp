...
<section class="profile-page">
  ...
  <div class="profile-header">
    <div class="profile-header-bg">
      <img src="{{ asset('public/assets/student_login/img/profile_bg.avif') }}" alt="Background">
      
    </div>

    <div class="profile-header-content">
      <div class="profile-header-text">
        <h5 class="fw-bold text-white mb-0" data-field="first_name">{{$getUser->first_name ?? ''}}</h5>
        <p class="text-white mb-0 small">{{$getSetting->name ?? ''}}</p>
        <small class="text-white-50">{{$getUser['ClassTypes']['name']}}</small>
      </div>

      <!-- ðŸ”¹ Profile Photo -->
      <div class="profile-photo text-center mt-3">
        <div class="circle position-relative d-inline-block"> 
            <img id="profileImage" src="{{ env('IMAGE_SHOW_PATH') . '/profile/' . $getUser['image'] }}" onerror="this.onerror=null; this.src='{{ asset('public/assets/student_login/img/user_icon.png') }}'">
  ...
  <div class="profile-details">
    <div class="info-grid">
      <div class="info-row">
        <div class="info-col">
          <label>Name</label>
          <p data-field="first_name">{{$data->first_name ?? ''}}</p>
        </div>
        <div class="info-col">
          <label>Mobile No.</label>
          <p data-field="mobile">{{$data->mobile ?? ''}}</p>
        </div>
        ...
        <div class="info-row single">
         <div class="info-col">
          <label>E-mail</label>
        <p data-field="email">{{$data->email ?? ''}}</p>
        </div>
        ...
        <div class="info-col">
          <label>Address</label>
          <p data-field="address">{{$data->address ?? ''}}</p>
        </div>
      </div>
      ...
      <div class="info-row">
        <div class="info-col">
          ...
          <p data-field="mother_name">{{$data->mother_name ?? ''}}</p>
        </div>
         <div class="info-col">
          <label>Father's Name</label>
          <p data-field="father_name">{{$data->father_name ?? ''}}</p>
        </div>
      ...
      <div class="info-row">
        <div class="info-col">
          ...
          <p data-field="father_mobile">{{$data->father_mobile ?? ''}}</p>
        </div>
       <div class="info-col">
          <label>Mother's Mobile No.</label>
          <p data-field="mother_mobile">{{$data->mother_mob ?? ''}}</p>
        </div>
      </div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  .then(data => {
    if(data.status === "success"){
      profileImage.src = "{{ asset('public/assets/student_login/img/user_icon.png') }}";
