...
const cfg = window.SAAS_CFG || {};
  ...
  if (cfg.amc?.enabled && ['critical','grace','warning'].includes(cfg.amc.state)) {
      showBanner("<marquee>⚠️ Software Is Expiring Soon @if(!empty(Session::get('amc_date'))) [{{ date('d-m-Y', strtotime(Session::get('amc_date'))) }}] @endif - Renew Your AMC</marquee>", "alert-info");
