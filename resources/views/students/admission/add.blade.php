				...
				<div class="col-12 col-md-12">
					<div class="card card-outline card-orange">
						<div class="card-header bg-primary">
							<h3 class="card-title"><i class="fa fa-address-book-o"></i> &nbsp;{{ __('Students Admission') }}</h3>
							<div class="card-tools">
								<a href="{{url('admissionView')}}" class="btn btn-primary  btn-sm {{ Helper::permissioncheck(3)->view ? '' : 'd-none' }}"><i class="fa fa-eye"></i> <span class="Display_none_mobile"> {{ __('common.View') }} </span></a>
						...
						<form id="form-submit" action="{{ url('admissionAdd') }}" method="post" enctype="multipart/form-data">
							...
							<div class="row m-2">
								...
								<div class="col-md-3 {{ $student_fields->contains('image') ? '' : 'd-none' }}">
									<lable>{{ __('student.Student Photo') }}
									@if($student_fields_required['image'] == 0)
                                                    <span style="color:red;">*</span>
                                                @endif
                                                </lable>
							...
							@if(Session::get('student_count') >= Session::get('register_student'))
								<button type="submit" class="btn btn-primary btn-submit"  id="submitButton">{{ __('common.submit') }}</button><br><br>
							@endif
							</div>
						</form>
					</div>
