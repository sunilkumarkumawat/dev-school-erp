...
body{
    background:url('{{ env('IMAGE_SHOW_PATH').'default/Icon_images/rm347-porpla-01.jpg' }}') no-repeat center/cover;
    min-height:100vh;
    display:flex;
...
<div class="card">
    ...
    <form action="{{ url('newRegistration') }}" method="POST" id="form-submit">
        @csrf

        <div class="form-group">
            <label>Software Token *</label>
            <input type="text" name="softwareTokenNo"
                   placeholder="Enter software activation token (e.g. RUK-XXXX-XXXX)" value="{{env('SOFTWARE_TOKEN_NO')}}">
        </div>

        <div class="form-group">
            <label>Database Name *</label>
            <input type="text" name="dbDatabase"
                   placeholder="Enter database name (e.g. school_management)" value="{{env('DB_DATABASE')}}">
        </div>

        <div class="form-group">
            <label>Database Username *</label>
            <input type="text" name="dbUsername"
                   placeholder="Enter database username (e.g. root)" value="{{env('DB_USERNAME')}}">
        </div>
        ...
        <div class="form-group">
            ...
            <input type="text" name="imageUploadPath"
                   value="/home/{{ $domain }}/public_html/{{ $subdomain }}/schoolimage/"
