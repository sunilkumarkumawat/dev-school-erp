...
<div class="page">
    <div class="header">
        <table class="no-border">
            <tr>
                ...
                <td class="meta right">Period: {{ \Carbon\Carbon::parse($monthStart)->format('d/m/Y') }} to {{ \Carbon\Carbon::parse($rangeEnd)->format('d/m/Y') }}</td>
            </tr>
            <tr>
                <td class="meta">{{ trim(($member->first_name ?? '').' '.($member->last_name ?? '')) }} ({{ $displayUniqueId ?? $uniqueId }}) | {{ $monthLabel }} {{ $year }}</td>
                <td class="meta right">Role: {{ $roleName }}</td>
            </tr>
        </table>
    </div>
        ...
        <table>
            <tr>
                <td>
                    ...
                    <div class="value">{{ number_format((float)$monthlySalary, 2) }}</div>
                </td>
                <td>
                ...
                <td>
                    ...
                    <div class="value">{{ number_format((float)$perDay, 2) }}</div>
                </td>
                <td>
                    <div class="stat">Paid Days</div>
                    <div class="value">{{ rtrim(rtrim(number_format((float)$paidDays, 2), '0'), '.') }}</div>
                </td>
            </tr>
            <tr>
                <td>
                    <div class="stat">Gross (Attendance)</div>
                    <div class="value">{{ number_format((float)$gross, 2) }}</div>
                </td>
                <td>
                    <div class="stat">Manual Deductions</div>
                    <div class="value">{{ number_format((float)$manualTotal, 2) }}</div>
                </td>
                <td>
                    <div class="stat">Loan/Advance</div>
                    <div class="value">{{ number_format((float)$loanTotal, 2) }}</div>
                </td>
                <td>
                    <div class="stat green">Net Payable</div>
                    <div class="value green">{{ number_format((float)$net, 2) }}</div>
                </td>
            </tr>
            <tr>
                <td colspan="3">
                    <div class="stat">Total Deductions</div>
                    <div class="value">{{ number_format((float)$deductionTotal, 2) }}</div>
                </td>
                <td>
                    <div class="stat">Leave Allowed</div>
                    <div class="value">{{ $payrollSetting->paid_leave_limit === null ? 'Unlimited' : $payrollSetting->paid_leave_limit.' days' }}</div>
                </td>
            </tr>
        </table>
    </div>
    ...
    <div class="section" style="padding-top:0;">
        ...
        <table>
            <tr>
                <td>Present: <b>{{ $countsByStatus['present'] }}</b></td>
                <td>Late: <b>{{ $countsByStatus['late'] }}</b></td>
                <td>Early Out: <b>{{ $countsByStatus['early_out'] }}</b></td>
                <td>Half Day: <b>{{ $countsByStatus['halfday'] }}</b></td>
            </tr>
            <tr>
                <td>Absent: <b>{{ $countsByStatus['absent'] }}</b></td>
                <td>Holiday: <b>{{ $countsByStatus['holiday'] }}</b></td>
                <td>Leave: <b>{{ $countsByStatus['leave'] }}</b></td>
                <td>&nbsp;</td>
            </tr>
        </table>
    </div>
        ...
        <table>
            ...
            @for($i = 0; $i < $maxRows; $i++)
                ...
                <tr>
                    <td>{{ $m->title ?? '-' }}</td>
                    <td>{{ $m->remark ?? '-' }}</td>
                    <td class="right">{{ $m ? number_format((float)$m->amount, 2) : '-' }}</td>
                    <td>{{ $l ? ucfirst($l->type ?? 'loan') : '-' }}</td>
                    <td>{{ $l->title ?? '-' }}</td>
                    <td class="right">{{ $l ? number_format((float)$l->amount, 2) : '-' }}</td>
                </tr>
            @endfor
        </table>
