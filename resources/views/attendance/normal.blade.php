            ...
            <div class="card card-outline card-orange">
                ...
                <div class="card-body">
                    ...
                    <div class="tab-content mt-3" id="attendanceTabContentNormal">
                        <div class="tab-pane fade {{ ($activeTab ?? 'students') === 'students' ? 'show active' : '' }}" id="students-normal" role="tabpanel">
                            <form method="post" action="{{ url('attendance/mark?tab=students&date='.$selectedDate.'&class_type_id='.request('class_type_id')) }}">
                                ...
                                <input type="hidden" name="class_type_id" id="classFilterInputNormal" value="{{ request('class_type_id') }}">
                                    <div class="ml-auto d-flex align-items-center" style="gap:8px;">
                                        <select id="bulkStatusStudentNormal" class="form-control form-control-sm att-filter" @if($holidayLock) disabled @endif>
                                        ...
                                        <button class="btn btn-sm btn-outline-secondary" type="button" id="applyBulkStudentNormal" @if($holidayLock) disabled @endif>Apply</button>
                                    ...
                                    <table class="table table-bordered att-table" id="studentsTableNormal">
                                        ...
                                        <tbody>
                                            @foreach($students as $i => $student)
                                                ...
                                                @endphp
                                                <tr class="student-row-normal" data-class="{{ $student->class_type_id }}">
                                                    <td>{{ $uniqueId }}</td>
                                                    ...
                                                    <td>
                                                        ...
                                                        <select class="form-control form-control-sm status-select-normal" name="rows[{{ $i }}][status]" @if($holidayLock) disabled @endif>
                        ...
                        <div class="tab-pane fade {{ ($activeTab ?? 'students') === 'staff' ? 'show active' : '' }}" id="staff-normal" role="tabpanel">
                            <form method="post" action="{{ url('attendance/mark?tab=staff&date='.$selectedDate.'&role_id='.request('role_id')) }}">
                                ...
                                <input type="hidden" name="role_id" id="roleFilterInputNormal" value="{{ request('role_id') }}">
                                    <div class="ml-auto d-flex align-items-center" style="gap:8px;">
                                        <select id="bulkStatusStaffNormal" class="form-control form-control-sm att-filter" @if($holidayLock) disabled @endif>
                                            <option value="">Bulk Status</option>
                                            <option value="present">Present</option>
                                            <option value="absent">Absent</option>
                                            <option value="leave">Leave</option>
                                        ...
                                        <button class="btn btn-sm btn-outline-secondary" type="button" id="applyBulkStaffNormal" @if($holidayLock) disabled @endif>Apply</button>
                                        ...
                                        <tbody>
                                            @foreach($staff as $i => $member)
                                                @php
                                                    ...
                                                    $statusVal = $mark->status ?? '';
                                                @endphp
                                                <tr class="staff-row-normal" data-role="{{ $member->role_id }}">
                                                    <td>{{ $uniqueId }}</td>
