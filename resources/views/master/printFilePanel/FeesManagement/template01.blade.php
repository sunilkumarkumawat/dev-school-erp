...
<head>
  ...
  @php
    ...
    $logoPath =  env('IMAGE_SHOW_PATH').'setting/left_logo/'.$getSetting['left_logo'];
//dd($logoPath);
if (!empty($getSetting['left_logo'])) {
    $type = pathinfo($logoPath, PATHINFO_EXTENSION);
    $Img_data = file_get_contents($logoPath);
    $base64_logo = 'data:image/' . $type . ';base64,' . base64_encode($Img_data);
} else {
    // fallback logo
    $fallbackPath =  env('IMAGE_SHOW_PATH').'/default/rukmanisoft_logo.png';
    $type = pathinfo($fallbackPath, PATHINFO_EXTENSION);
    $Img_data = file_get_contents($fallbackPath);
    $base64_logo = 'data:image/' . $type . ';base64,' . base64_encode($Img_data);
}
...
<table style="border-bottom: 0px solid;width:100%;" >
	...
	<tr>
      <td class="border_none" rowspan='1' colspan="12"width='20%'>
          <img  rowspan="4" src="{{ $base64_logo }}" style="width: 97px;">
