 ...
 @if(!empty($data))
    @foreach($data as $key => $item)
      @php                               
                $sub = DB::table('examination_schedules')->select('examination_schedules.*','subject.name as subject_name')
                      ...
                      ->where("examination_schedules.exam_id", $exam_id)
                      ->where('subject.deleted_at',null)
                       ->where("subject.session_id", Session::get("session_id"))
                      ->where('examination_schedules.date','!=','1970-01-01')
                      ->orderBy('examination_schedules.date','ASC')->get();  
                      $exam = null;
                        if ($sub->isNotEmpty()) {
                            $exam = DB::table('exams')->where('id', $sub->first()->exam_id)->first();
                        }
            @endphp  
    ...
    <table style="border: 1px solid black;">
        <tbody class="bg_theme">
            <tr>
                <td rowspan='2' width='20%' class="rtr">
                    <img src="{{ env('IMAGE_SHOW_PATH').'/setting/left_logo/'.$getSetting['left_logo'] }}" onerror="this.src='{{ env('IMAGE_SHOW_PATH').'/default/rukmani_logo.png' }}'" width="100px">
                </td>
                <td width='75%' style="font-size:22px;text-align:center;text-transform:capitalize"><span><strong>{{$getSetting['name'] ?? ''}}</strong></span>
                </td>
            ...
            <tr style="border: 1px solid black;">
                <td colspan="12">
                    <p
                        ...
                        (Academic Session {{$session->from_year ?? '' }}{{"-"}}{{$session->to_year ?? ''}}) - {{$exam->name ?? ''}}
                    </p>
                </td>
            </tr>
        </tfoot>
    ...
    <table id='personal_detail'>
        <tbody>
            <tr>
                <th class="bg_theme plt">Exam Roll No. :- {{ $item->roll_no ?? '' }}</th>
